			...
			<div class="panel panel-default">
				<div class="panel-heading" style="height: 50px;">
					<div style="float: right;">
					    <a href="<?=base_url(array("admin.php", "labels", "order"));?>" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-sort"></span> Setează ordinea</a>
						<a href="<?=base_url(array("admin.php", "labels", "add"));?>" class="btn btn-info btn-sm">+ Adaugă o etichetă</a>
					</div>
					<?=$title?>
				</div>
				<!-- /.panel-heading -->
						...
						<table class="table table-striped table-bordered table-hover" id="dataTables-example">
							...
							<?php foreach ($labels as $item): ?>
								<tr class="odd gradeX" id="tr<?=$item['id']?>">
								    <td><span class="fa fa-circle" style="color: <?=$item['color']?>"></span></td>
									<td>
										<a href="<?=base_url(array("admin.php", "labels", "add", $item['id']));?>" class="btn btn-link"><?=htmlspecialchars($item['title'])?></a></td>
									</td>
									...
									<td align="center" nowrap>
										<a href="<?=base_url(array("admin.php", "labels", "add", $item['id']));?>" class="btn btn-primary">Editează</a>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="gridSystemModalLabel">Şterge eticheta</h4>
			</div>
			<div class="modal-body">
				Eşti sigur că doreşti să ştergi eticheta <b><span id="modal_content"></span></b>?
			</div>
...
function delete_record_confirm() {
	...
	$.ajax({
        url: "<?=base_url(array("admin.php", "labels", "delete"));?>",
        type :"POST",
        data: {
        	id_label:id_label
        },
        success: function(response) {
        	$("#tr"+id_label).slideUp();
			$.jGrowl("Eticheta a fost ştearsă");
        }
    });
}
</script>